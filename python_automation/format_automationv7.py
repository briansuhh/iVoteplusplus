...
# Function to determine Level based on index
def get_level(index):
    if 1 <= index <= 114 or 411 <= index <= 858:
        return "Level 1"
    elif 115 <= index <= 401 or 859 <= index <= 1511:
        return "Level 2"
    elif 402 <= index <= 410 or 1512 <= index <= 1563:
        return "Level 3"
    return "Level 1"  # Default (should never be used)
...
def format_name(full_name, index):
    ...
    # Remove period from "JR." or "Sr." in surname for email purposes
    clean_surname = surname.replace(".", "")

    # Generate email using initials + full surname
    first_initials = "".join([word[0].lower() for word in first_name.split()])
    email_base = f"{first_initials}{middle_initial.lower()}{clean_surname.lower().replace(' ', '')}"
    
    # Handle duplicate emails by appending a counter
    email_suffix = ""
    if email_count[email_base] > 0:
        email_suffix = str(email_count[email_base])  # Add 1, 2, etc.
    
    email = f"{email_base}{email_suffix}@pup.edu.ph"
...
for idx, entry in enumerate(member_data, start=1):
    ...
    level = get_level(idx)
    member_detail_data.append([idx, code, "area", level])
